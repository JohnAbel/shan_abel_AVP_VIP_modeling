...
if __name__ == '__main__':
    ...
    trajectories = model.run(show_labels=False, seed = 0)
    
    wt_ts = trajectories[0][:,0]
    wt_avpsol = trajectories[0][:,1:(80+1)]
    wt_vipsol = trajectories[0][:,(80+1):(160+1)]
    wt_navsol = trajectories[0][:,(160+1):]
    ...
    wt_T = period
    ax.plot(wt_ts/wt_T, wt_vipsol[:,::4], 'darkorange', alpha=0.1, ls='--')
    ax.plot(wt_ts/wt_T, wt_navsol[:,::3], 'goldenrod', alpha=0.1, ls='-.')
    ax.plot(wt_ts/wt_T, wt_avpsol[:,::4], 'purple', alpha=0.1)
